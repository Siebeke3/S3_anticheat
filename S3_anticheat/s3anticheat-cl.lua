...
AddEventHandler('Anticheat:WLReturn', function(wlstatus)
	...
	--whitelisted = true   ##Aanzetten als je een whitelist wilt
	if whitelisted == true then
		print ('Speler is gewhitelisted.')
	else
		print ('Speler is niet gewhitelisted')
	end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(500)
		if (AntiCheat == true and whitelisted == false and whiteCheck == false)then
			for _,theWeapon in ipairs(Config.Wapenblacklist) do
				Wait(1)
...
Citizen.CreateThread(function()
	while true do
        ...
        if (AntiCheat == true and whitelisted == false and whiteCheck == false)then
			...
			for i=1,#Config.Objectenblacklist do
				if GetEntityModel(object) == GetHashKey(Config.Objectenblacklist[i]) then
		...
		if (AntiCheat == true and whitelisted == false and whiteCheck == false) then
			if IsPedJumping(PlayerPedId()) then
				...
				if jumplength > 10.0 then
					print('U bent gedetecteerd met superjump')
...
AddEventHandler('AntiCheat:Toggle', function()
    if (AntiCheat == false) then
        AntiCheat = true
        AntiCheatStatus = "~b~Actief"
        anticheatm = "~y~S3 AntiCheat ~b~Ingeschakeld"
        Citizen.Wait(5000)
        anticheatm = false
    	else
		AntiCheat = false
        AntiCheatStatus = "~r~Inactief"
        anticheatm = "~y~S3 AntiCheat ~r~Uitgeschakeld"
        PedStatus = "OFF"
        Citizen.Wait(5000)
        anticheatm = false
	end
...
Citizen.CreateThread( function()
	while true do
        ...
        if whitelisted == true then
			rgb = RGBRainbow(2)
			ACstatus(0.19, 0.88, 1.0,1.0,0.4, "S3 AntiCheat: "..AntiCheatStatus, rgb.r, rgb.g, rgb.b, 255, 200)
			ACstatus(0.19, 0.88 + 0.03, 1.0,1.0,0.35, "Burgers: ~r~"..PedStatus, 255, 255, 255, 200)
		end
	end
end)
...
function isCarBlacklisted(model)
	for _, blacklistedCar in pairs(Config.Voetuigblacklist) do
