...
AddEventHandler('AntiCheat:Cars', function(blacklistedCar)
	...
	bandata = {}
	bandata.reason = _U('drop_player_blcars_notification', blcar)..Config.Discord -- ban + reden van ban
	bandata.period = '0' -- dagen, 0 voor permanent
...
AddEventHandler('AntiCheat:Jump', function()
	...
	bandata = {}
	bandata.reason = _U('drop_player_superjump_notification')..Config.Discord -- ban + reden van ban
	bandata.period = '0' -- dagen, 0 voor permanent
	TriggerEvent('Anticheat:AutoBan', _source, bandata)
end)
...
TriggerEvent('es:addGroupCommand', 'banreload', Config.permissie, function (source)
  BanListLoad        = false
...
TriggerEvent('es:addGroupCommand', 'banhistory', Config.permissie, function (source, args, user)
...
TriggerEvent('es:addGroupCommand', 'unban', Config.permissie, function (source, args, user)
  if args[1] ~= nil then
...
TriggerEvent('es:addGroupCommand', 'ban', Config.permissie, function (source, args, user)
	local identifier
	local license
...
TriggerEvent('es:addGroupCommand', 'banoffline', Config.permissie, function (source, args, user)
	if args ~= "" then
...
TriggerEvent('es:addGroupCommand', 'reason', Config.permissie, function (source, args, user)
	...
	if name ~= "" then
		if duree ~= nil and duree < 365 then
			...
			MySQL.Async.fetchAll('SELECT * FROM baninformatie WHERE playername = @playername', 
			{
				['@playername'] = name
			}, function(data)
...
AddEventHandler('bansql:sendMessage', function(source, message)
	if source ~= 0 then
		TriggerClientEvent('chat:addMessage', source, { args = { 'banlist', message } } )
	else
...
AddEventHandler('es:playerLoaded',function(source)
  ...
  Wait(5000)
	...
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		...
		MySQL.Async.fetchAll('SELECT * FROM `baninformatie` WHERE `identifier` = @identifier', {
			['@identifier'] = steamID
		}, function(data)
		local found = false
			for i=1, #data, 1 do
			...
			if not found then
				MySQL.Async.execute('INSERT INTO baninformatie (identifier,license,liveid,xblid,discord,playerip,playername) VALUES (@identifier,@license,@liveid,@xblid,@discord,@playerip,@playername)', 
					{ 
					['@identifier'] = steamID,
					['@license']    = license,
					['@liveid']     = liveid,
				...
				MySQL.Async.execute('UPDATE `baninformatie` SET `license` = @license, `liveid` = @liveid, `xblid` = @xblid, `discord` = @discord, `playerip` = @playerip, `playername` = @playername WHERE `identifier` = @identifier', 
					{ 
					['@identifier'] = steamID,
					['@license']    = license,
					['@liveid']     = liveid,
